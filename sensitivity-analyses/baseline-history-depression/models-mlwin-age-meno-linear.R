...
# Effect of age at menopause over age on depression

# Model depression over chronological age
model1 <- runMLwiN(epds_prorated ~ 1 + age*age_menopause + age_sq*age_menopause
...
pred_df_model1 <- data.frame(age = pred_model1$age,
                             ...
                             predicted = predicted_model1$fit,
                             CI_low = CI_low_model1,
                             CI_high = CI_high_model1)
...
ggsave("Final plots/sensitivity/history_depression/Trajectory of Depression Over Chronological Age (age meno contin).png", width = 11.69, height = 8.27,bg = "white")
...
ggsave("Final plots/sensitivity/history_depression/Trajectory of Depression Over Chronological Age (age meno contin + CIs).png", width = 11.69, height = 8.27, units = "in")


model2 <- runMLwiN(epds_prorated ~ 1 + age*age_meno_cat + age_sq*age_meno_cat
                   + ethnicity + social_class + education + age_menarche
                   + material_hardship + social_support + smoking_status + bmi + alcohol_intake
                   + history_depression_d
                   + (1 + age | aln) + (1 + age| time_point) ,
...
pred_df_model2 <- data.frame(
  ...
  predicted = c(predicted_model2_under48$fit,predicted_model2_over48$fit),
  CI_low = c(CI_low_model2_under48, CI_low_model2_over48),
  CI_high = c(CI_high_model2_under48,CI_high_model2_over48),
...
ggsave("Final plots/sensitivity/history_depression/Age at menopause categorical.png", width = 11.69, height = 8.27, units = "in", bg = "white")
...
ggsave("Final plots/sensitivity/history_depression/Age at menopause categorical with CIs.png", width = 11.69, height = 8.27, units = "in", bg = "white")
