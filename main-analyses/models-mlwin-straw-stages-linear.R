...
model1 <- runMLwiN(epds_prorated ~ 1 + age 
                   + stage + age:stage 
...
pred_model1_pre$age_sq <- pred_model1_pre$age^2
...
pred_model1_peri$age_sq <- pred_model1_peri$age^2
...
pred_model1_post$age_sq <- pred_model1_post$age^2
...
CI_low_model1_pre <- predicted_model1_pre$fit - 1.96 * predicted_model1_pre$se.fit
CI_high_model1_pre <- predicted_model1_pre$fit + 1.96 * predicted_model1_pre$se.fit

CI_low_model1_peri <- predicted_model1_peri$fit - 1.96 * predicted_model1_peri$se.fit
CI_high_model1_peri <- predicted_model1_peri$fit + 1.96 * predicted_model1_peri$se.fit

CI_low_model1_post <- predicted_model1_post$fit - 1.96 * predicted_model1_post$se.fit
CI_high_model1_post <- predicted_model1_post$fit + 1.96 * predicted_model1_post$se.fit


# Combine predictions into one data frame
pred_df <- data.frame(
  age = c(pred_model1_pre$age, pred_model1_peri$age, pred_model1_post$age),
  predicted = c(predicted_model1_pre$fit, predicted_model1_peri$fit, predicted_model1_post$fit),
...
ggsalegend.position.inside = ggsave("Final plots/Trajectory of Depression Over Chronological Age (straw stages).png", width = 11.69, height = 8.27, units = "in", bg = "white")

# Model STRAW stages over age with age_sq
model2 <- runMLwiN(epds_prorated ~ 1 + age + age_sq + stage
                   + age:stage + age_sq:stage
                   + ethnicity + social_class + education + age_menarche
                   + material_hardship + social_support + smoking_status + bmi + alcohol_intake
                   + (1 + age | aln) + (1 + age | time_point),
                   D = "Normal",
...
pred_model2_pre$age_sq <- pred_model2_pre$age^2

pred_model2_peri <- data.frame(
  age = seq(from = quantiles$lower_bound[which(quantiles$stage == "menopause_transition")], to = quantiles$upper_bound[which(quantiles$stage == "menopause_transition")], length.out = 100),
  stage = factor("menopause_transition", levels = levels(df$stage)),
...
pred_model2_peri$age_sq <- pred_model2_peri$age^2

pred_model2_post <- data.frame(
  age = seq(from = quantiles$lower_bound[which(quantiles$stage == "post_menopause")], to = quantiles$upper_bound[which(quantiles$stage == "post_menopause")], length.out = 100),
  stage = factor("post_menopause", levels = levels(df$stage)),
...
pred_model2_post$age_sq <- pred_model2_post$age^2

# Predict values
predicted_model2_pre <- predict(model2, newdata = pred_model2_pre, type = "link", se.fit = T)
predicted_model2_peri <- predict(model2, newdata = pred_model2_peri, type = "link", se.fit = T)
predicted_model2_post <- predict(model2, newdata = pred_model2_post, type = "link", se.fit = T)

# Calculate the confidence intervals
CI_low_model2_pre <- predicted_model2_pre$fit - 1.96 * predicted_model2_pre$se.fit
CI_high_model2_pre <- predicted_model2_pre$fit + 1.96 * predicted_model2_pre$se.fit

CI_low_model2_peri <- predicted_model2_peri$fit - 1.96 * predicted_model2_peri$se.fit
CI_high_model2_peri <- predicted_model2_peri$fit + 1.96 * predicted_model2_peri$se.fit

CI_low_model2_post <- predicted_model2_post$fit - 1.96 * predicted_model2_post$se.fit
CI_high_model2_post <- predicted_model2_post$fit + 1.96 * predicted_model2_post$se.fit


# Combine predictions into one data frame
pred_df <- data.frame(
  age = c(pred_model2_pre$age, pred_model2_peri$age, pred_model2_post$age),
  predicted = c(predicted_model2_pre$fit, predicted_model2_peri$fit, predicted_model2_post$fit),
  CI_low = c(CI_low_model2_pre, CI_low_model2_peri, CI_low_model2_post),
  CI_high = c(CI_high_model2_pre, CI_high_model2_peri, CI_high_model2_post),
  stage = factor(c(rep("Reproductive", 100), rep("Perimenopause", 100), rep("Postmenopause", 100)))
)
pred_df$stage <- factor(pred_df$stage,
                        levels = c("Reproductive","Perimenopause","Postmenopause"))
...
ggplot(pred_df, aes(x = age, y = predicted, colour = stage, fill = stage)) +
  ...
  scale_colour_manual(values = c(
    "Reproductive" = "#009E73",
    "Perimenopause" = "#E69F00",
    "Postmenopause" = "#CC79A7"
  )) +
  scale_fill_manual(values = c(
    "Reproductive" = "#009E73",
    "Perimenopause" = "#E69F00",
...
ggsave("Final plots/Trajectory of Depression Over Chronological Age (straw stages + age sq).png", width = 11.69, height = 8.27, units = "in",bg = "white")
...
pred_df <- data.frame(
  ...
  CI_low = c(CI_low_model1_pre, CI_low_model1_peri, CI_low_model1_post),
  CI_high = c(CI_high_model1_pre, CI_high_model1_peri, CI_high_model1_post),
  stage = factor(c(rep("Reproductive", 21), rep("Perimenopause", 11), rep("Postmenopause", 11)))
)
pred_df$stage <- factor(pred_df$stage,
                        levels = c("Reproductive","Perimenopause","Postmenopause"))
View(pred_df)
...
model1@FP[2]+model1@FP[5]
0.10147 + 0.06894 # lower CI
0.12591 +0.13303 # upper CI

# Postmenopause
model1@FP[2]+model1@FP[6]
0.10147 + 0.09156 # lower CI
0.12591 + 0.13574 # upper CI
model1
