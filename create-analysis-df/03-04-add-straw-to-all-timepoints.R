...
length(unique(df$aln[which(df$exclude_participant == 1)]))
...
colnames <- colnames[-which(colnames %in% c("aln","time_point","age_menopause","fmp","periods_stopped_surgery","periods_stopped_surgery_date","periods_stopped_surgery_age"))]
...
table(df$stage)
saveRDS(df, paste0(filestore,"all_timepoints_with_straw.rds"))
