...
pred_df <- data.frame(
  ...
  stage = factor(c(rep("Premenopause", 100), rep("Perimenopause", 100), rep("Postmenopause", 100)))
)
pred_df$stage <- factor(pred_df$stage,
                        levels = c("Premenopause","Perimenopause","Postmenopause"))
...
ggsalegend.position.inside = ggsave("Final plots/sensitivity/history_depression/Trajectory of Depression Over Chronological Age (straw stages).png", width = 11.69, height = 8.27, units = "in", bg = "white")
