...
repro_age_lower <- as.numeric(quantile(df$repro_age, 0.05,na.rm = T))
repro_age_upper <- as.numeric(quantile(df$repro_age, 0.95,na.rm = T))

df <- df %>% filter(repro_age >= repro_age_lower
                    & repro_age <= repro_age_upper)
